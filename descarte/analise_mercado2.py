...
CONFIG_RELATORIO = {
    ...
    "secoes": [
        ...
        "Indicadores Econômicos", "Benchmarking", "Análise SWOT", "5 Forças de Porter", "CBD (Cost Breakdown)", "SCA (Should Cost Analysis)", "LPP (Line Performance Pricing)",
        "Perguntas para Clientes e Fornecedores", "SLAs e Multas", "Critérios de Seleção de Fornecedores",
        "Estrutura de RFP", "Análise de Riscos", "Alavancas de Negociação e BATNA",
        "Momento Ideal para Negociação", "Tendências ESG", "Conclusão"
    ]
}
...
# Função para exportar Markdown para Word


def exportar_para_word(conteudo_md, setor, arquivo_saida):
    try:
        html = markdown.markdown(conteudo_md, extensions=['extra', 'tables'])
        doc = Document()
        doc.add_heading(f'Relatório de Análise de Mercado: {setor}', 0)
        doc.add_paragraph('Preparado por Vorätte Consultoria')
      
        # Adicionar conteúdo (simplificado; parse HTML para formatação completa)
        ...
        logging.error(f"Erro ao exportar para Word: {e}")
        raise
...
redacao_relatorio = Task(
    description=(
        "Gerar um relatório de {CONFIG_RELATORIO['contagem_palavras']} palavras em Markdown para {setor}, cobrindo todas as seções: {', '.join(CONFIG_RELATORIO['secoes'])}.\n"
...
def gerar_relatorio(setor, regiao, empresa):
    try:
        ...
        # Gerar gráficos (dados de exemplo)
        swot_dados = {"Escalabilidade": 9, "Custo": 6, "Inovação": 8, "Risco": 5}
        porter_dados = {"Poder dos Fornecedores": 5, "Poder dos Compradores": 8, "Novos Entrantes": 3, "Substitutos": 6, "Rivalidade": 9}
        ...
        exportar_para_word(str(resultado), setor, arquivo_saida)
         ...
         # Salvar Markdown
        arquivo_md = f"relatorio_{setor.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.md"
        with open(arquivo_md, "w", encoding="utf-8") as f:
            f.write(str(resultado))
        logging.info(f"Relatório Markdown salvo como {arquivo_md}")
